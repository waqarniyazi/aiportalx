...
const modelSchema = new Schema<IModel>(
  {
    ...
    'Organization categorization (from Organization)': { type: String, default: '' },
  },
  { collection: 'models' }
);
...
app.get('/api/models', async (req: Request, res: Response) => {
  try {
    const { Task, Domain, Organization, Country } = req.query as Record<string, string | string[]>;
    ...
    if (Country) query['Country (from Organization)'] = { $in: ensureArray(Country) };
...
app.get('/api/models/:organization/:model', async (req: Request, res: Response) => {
  try {
